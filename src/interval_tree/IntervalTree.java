...
public class IntervalTree {
    ...
    private class TreeNode {
        ...
        TreeNode(Interval interval, int color) {
    ...
    public IntervalTree() {
        //创建NIL节点
        nil = new TreeNode(new Interval(-1, -1), BLACK_COLOR);
        nil.max = 0;//max设置为0
        //刚开始没有元素，root就是根节点
        root = nil;
    }
    ...
    /**
     * @param interval 步骤1：按照二叉树的方式插入
     *                 步骤2：对二茶树进行调色和旋转
     */
    public void RBInsert(Interval interval) {

        //创建节点，颜色初始为红色
        TreeNode newNode = new TreeNode(interval, RED_COLOR);
        TreeNode x = root;
        TreeNode y = nil;//记录父节点,初始化为nil，保证空树的时候不异常
        while (x != nil) {
            y = x;//记录父节点
            //更新max
            x.max = Math.max(x.max, interval.high);
            //如果x大于新节点的值，那么去左边
            if (newNode.interval.low < x.interval.low)
                x = x.left;
                //否则去右边
            else
                x = x.right;
        ...
        if (y == nil)
            ...
            if (newNode.interval.low < y.interval.low)
                y.left = newNode;
            else
                y.right = newNode;
            //父指针指向y
     ...
     * @param i
     */
    public Interval RBSearch(Interval i) {
        return binarySearch(root, i);
    }
     ...
     * @return
     */
    private Interval binarySearch(TreeNode node, Interval xInterval) {
        if (node == nil)
            return null;
        if (isOverride(node.interval, xInterval))
            return node.interval;
        //左子树不为空，并且左子树的high大于待查询区间的low
        if (node.left != nil && node.interval.high >= xInterval.low)
            return binarySearch(node.left, xInterval);
            //否则去左边查找
        else
            return binarySearch(node.right, xInterval);
    }
    ...
    private void leftRotate(TreeNode x) {
        ...
        y.max = x.max;
        //调用两次max
        x.max = Math.max(y.left.max, Math.max(x.interval.high,x.left.max ));
    ...
    private void rightRotate(TreeNode x) {
        ...
        y.max = x.max;
        //调用两次max
        x.max = Math.max(y.right.max, Math.max(x.interval.high,x.right.max ));
    ...
    private boolean isOverride(Interval i1, Interval i2) {
    ...
    private void writeArray(TreeNode currNode, int rowIndex, int columnIndex, String[][] res, int treeDepth) {
        ...
        if(currNode.color==RED_COLOR)
            str=  "\033[31;4m" +currNode.interval.low + "\033[0m";
        else
            str=currNode.interval.low +"";
