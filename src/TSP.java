...
public class TSP {
    ...
    public int tsp(int[][] nums) {
        ...
        for (int i = 1; i < nums.length; i++) {
            visited[i] = true;
            minDistance = Math.min(minDistance, nums[0][i] + reve(i, 1));
            visited[i] = false;
    ...
    public int reve(int number, int count) {
        countTimes++;
        ...
        for (int i = 1; i < citys; i++) {
            ...
            if (visited[i] || i == number)
                ...
                if(distance[number][i]>=minDistance)
                    continue;
                res = Math.min(res, distance[number][i] + reve(i, count + 1));
                visited[i] = false;
            }
        }
        return res;
